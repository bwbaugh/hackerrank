...
-}
main :: IO ()
main = fmap (map read . words) getLine >>= print . fibModified

fibModified :: [Integer] -> Integer
fibModified [a, b, n] = head . drop (fromIntegral n - 1) $ series
  where
    series = a : b : zipWith t series (tail series)
    t n1 n2 = n1 + n2 ^ 2
