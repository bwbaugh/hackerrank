...
testCase = do
    n <- readLn
    case decentNumber n of
        [] -> putStrLn "-1"
        (x:_) -> putStrLn x

decentNumber :: Int -> [String]
decentNumber n = do
    let startFive = closestTo 3 n
    fives <- [startFive,startFive-3..0]
    let startThree = closestTo 5 (n - fives)
    threes <- [startThree,startThree-5..0]
    guard $ fives + threes == n
    return $ replicate fives 5 ++ replicate threes 3 >>= show

closestTo :: Integral a => a -> a -> a
closestTo n x = x - (x `mod` n)
